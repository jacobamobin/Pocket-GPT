...
function FAQItem({ q, a }) {
  ...
  return (
    <div className="border border-neural-border rounded-lg overflow-hidden">
      ...
      >
        <span className="text-sm text-slate-300 pr-2">{q}</span>
        <motion.svg
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.div
            ...
            transition={{ duration: 0.2 }}
          >
            <div className="px-3 py-2.5 text-sm text-slate-400 leading-relaxed border-t border-neural-border bg-neural-surface/50">
              {a}
            </div>
          </motion.div>
        )}
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-neural-border">
              <h2 className="text-base font-semibold text-slate-200">{content.title}</h2>
              <button
                onClick={() => dispatch({ type: 'CLOSE_INFO_DRAWER' })}
                className="text-slate-500 hover:text-white transition-colors text-xl leading-none
                           focus:outline-none focus:ring-1 focus:ring-blue-500/60 rounded"
                aria-label="Close"
              >
                &times;
              </button>
